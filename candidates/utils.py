        ...
        for row in reader:
            try:
                ...
                with transaction.atomic():
                    candidate = Candidate(
                        name=candidate_name, candidate_reference=candidate_reference
                    )
                    candidate.full_clean()
                    candidate.save()

                    score = Score(candidate=candidate, score=score)
                    score.full_clean()
                    score.save()
...
def read_json_data(json_file_path: str, output_file_path: str) -> None:
    ...
    with open(output_file_path, "w", newline="\n") as csv_file:
        ...
        for candidate_data in sorted_candidates:
            ...
            score_serializer = ScoreSerializer(data={"score": candidate_data["score"]})
